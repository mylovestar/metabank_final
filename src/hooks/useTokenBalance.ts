...
import { useCake, useErc721CollectionContract, useTokenContract } from './useContract'
...
export const useHandBalanceOfContract = (address: string) => {
  const { account } = useWeb3React()

  const contract = useErc721CollectionContract('0x7E82123bCb6465133D6E9E1Ad94d0115DE041b3D')
  const { data, status, ...rest } = useSWRContract(
    {
      contract,
      methodName: 'balanceOf',
