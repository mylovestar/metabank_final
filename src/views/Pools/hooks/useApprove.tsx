...
export const useApprovePoolNft = (lpContract: Contract, sousId, earningTokenSymbol) => {
  const { toastSuccess } = useToast()
  const { fetchWithCatchTxError, loading: pendingTx } = useCatchTxError()
  const { callWithGasPrice } = useCallWithGasPrice()
  const { t } = useTranslation()
  ...
  const handleApprove = useCallback(async () => {
    const receipt = await fetchWithCatchTxError(() => {
      return callWithGasPrice(lpContract, 'setApprovalForAll', [sousChefContract.address, true])
    })
    if (receipt?.status) {
      toastSuccess(
        t('Contract Enabled'),
  ...
  const handleApprove = async () => {
    ...
    if (receipt?.status) {
      toastSuccess(
        t('Contract Enabled'),
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('You can now stake in the %symbol% vault!', { symbol: 'HODLX' })}
        </ToastDescriptionWithTx>,
      )
      setLastUpdated()
    }
...
export const useApproveVaultPool = (lpContract: Contract, handContract: Contract, sousId, earningTokenSymbol) => {
  const { toastSuccess } = useToast()
  const { fetchWithCatchTxError, loading: pendingTx } = useCatchTxError()
  const { callWithGasPrice } = useCallWithGasPrice()
  const { t } = useTranslation()
  ...
  const handleApprove = useCallback(async () => {
    ...
    const receipt1 = await fetchWithCatchTxError(() => {
      return callWithGasPrice(handContract, 'setApprovalForAll', [sousChefContract.address, true])
    })
    if (receipt?.status && receipt1.status) {
      toastSuccess(
        t('Contract Enabled'),
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('You can now stake in the %symbol% pool!', { symbol: earningTokenSymbol })}
